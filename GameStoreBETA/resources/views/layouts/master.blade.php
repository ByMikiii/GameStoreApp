...
<body class="h-full overflow-y-scroll scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-bg-color">
  <header class="flex">
    ...
    <a href="/basket" class="flex hover:text-white">
    ...
    <form class="m-2 p-1" method="POST" action="{{ route('logout') }}">
        @csrf
        <li><button type="submit" class="w-7 h-7 mt-0.5 fill-zinc-400 hover:fill-red-600">
  ...
  <ul id="mobile-menu" class="border fixed w-full text-center bg-bg-color" style="display: none;">
    ...
    <li id="logged" class="list-item"><a  href="/profile">{{Auth::user()->name}}</a></li>


    <li class="list-item ml-auto"><a href="/" id="wallet">{{Auth::user()->wallet}} €</a></li>
    <form class="m-2 p-1" method="POST" action="{{ route('logout') }}">
        @csrf
...
<div class="w-full text-right mt-1">
  <button onclick="changeLang('sk')" class="font-semibold hover:text-tx-color <?php if(App::currentLocale() == 'sk'){echo "underline";}?>">SK</button>
  <button onclick="changeLang('en')" class="font-semibold mr-3 hover:text-tx-color <?php if(App::currentLocale() == 'en'){echo "underline";}?>">EN</button>
</div>
<?php endif; ?>
